    ...
    while (cards_read < NUM_CARDS)
    {
        ...
        The current card:
            values to insert for the current card: rank_value, and suit_value
             ...
             hand[0][SUIT] is the FIRST CARD'S SUIT

             array[index] = value
             hand[cards_read][0] = rank_value
             hand[cards_read][SUIT] = suit_value
            ...
            {
                if (hand[i][0] == rank_value && hand[i][SUIT] == suit_value)
                {
                    duplicate_card = true;
                    break;
                }
       ...
       */
        ...
        {
            if (hand[i][RANK] == rank_value && hand[i][SUIT] == suit_value)
            {
                duplicate_card = true;
                break;
            }
        ...
        hand[cards_read][RANK] = rank_value;
        hand[cards_read][SUIT] = suit_value;
        cards_read++;
    }
}
...
void analyze_hand(int hand[NUM_CARDS][2])
{
    ...
    for (int i = 0; i < NUM_CARDS - 1; i++)
    {
        int left_value = hand[i][RANK];
        int right_value = hand[i+1][RANK];

        if (left_value + 1 != right_value)
        {
            // if you don't have a normal straight, then also check for an ace-low straight
            // unsorted: a 5 4 3 2
            // sorted:   2 3 4 5 a
            // AND you're on the last iteration of the loop, then check for an ace-low straight, meaning that the last two cards are five(value=3) and ace(value=12)
            if (i == NUM_CARDS - 2)  // check for the final iteration
            {
                //check the last 2 cards for five and ace for an ace-low straight
                if (hand[NUM_CARDS - 2][RANK] == 3 && hand[NUM_CARDS - 1][RANK] == 12)
                {
    ...
    // if (straight == false && hand[0][0] == 0 && hand[SUIT][0] == 1 && hand[2][0] == 2 && hand[3][0] == 3 && hand[4][0] == 12)
    // {
    //     straight = true;
    // }
    ...
    // unsorted: a q j t k
    // sorted:   th jh qh kh ah
    if (straight && flush)
    {
        if (hand[0][RANK] ==  8) // 't' is rank_value 8
        {
            royal = true;
        }
    }
...
void print_hand(int hand[NUM_CARDS][2])
{
    ...
    for (int i = 0; i < NUM_CARDS; i++)
    {
        int rank_value = hand[i][RANK];
        int suit_value = hand[i][SUIT];
        printf("Card[%d]: ", i + 1);
...
void sort_hand(int hand[NUM_CARDS][2]) 
{
    ...
    for (int i = 0; i < NUM_CARDS - 1; i++)
    {
        ...
        {
            int left_hand_card = hand[j][RANK];
            int right_hand_card = hand[j + 1][RANK];
            if (left_hand_card > right_hand_card)
            {
                ...
                hand[j][RANK] = left_hand_card;
                hand[j + 1][RANK] = right_hand_card;
            }
        }
    }
}
